...
"""
This file contains the logic for loading data for all HanCD tasks.
"""
...
class CNTProcessor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["baby","beauty","car","comic","constellation","culture","design","digi","drama","dress","education","food","game","health","house","it","joke","lottery","manage","mass_communication","money","movie","music","news","pet","photo","science","sex","sports","star","travel","tv"]
...
class Medical_1Processor(DataProcessor):
...
class News_1Processor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["computer","recreation"]
...
class CnClickbaitProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf8') as f:
            reader = csv.reader(f, delimiter=',')
            for idx, row in enumerate(reader):
                label, headline, body, source = row
                text_a = headline.replace('\\', ' ')
                text_b = body.replace('\\', ' ')
                ...
                examples.append(example)
        return examples
class ClickbaitProcessor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["clickbait","news"]
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                examples.append(example)
        return examples
class News_2Processor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["computer","science"]
...
class News_3Processor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["computer","talk"]
...
class K_EProcessor(DataProcessor):
...
class AclProcessor(DataProcessor):
...
class MovielensProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        self.labels = ['great', 'good', 'common', 'bad', 'awfully']
...
PROCESSORS = {
    ...
    "mnli": MnliProcessor,
    "thu" : THUProcessor,
    "medical_1" : Medical_1Processor,
    "news_1" : News_1Processor,
    "news_2" : News_2Processor,
    "news_3" : News_3Processor,
    "acl" : AclProcessor,
    "cnt" : CNTProcessor,
    "reuter": ReuterProcessor,
    "newstitle": NewstitleProcessor,
    "AgnewsTitle":AgnewsTitleProcessor,
    "clickbait":ClickbaitProcessor,
    "clickbait":CnClickbaitProcessor
}
