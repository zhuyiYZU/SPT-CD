...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            # print(trains)  #(x, seq_len, mask, image_paths )
            # print('\t')
...
def test(config, model, test_iter):
    ...
    model.eval()
    start_time = time.time()
    metrics_arr, test_loss, test_report, test_confusion = evaluate(config, model, test_iter, test=True)
    msg = 'Test Loss: {0:>5.2},  Test Acc: {1:>6.2%}, Test Pre: {1:>6.2%}, Test Recall: {1:>6.2%}, Test F1: {1:>6.2%}'
    logger.log(msg.format(test_loss, metrics_arr[0]))
    # print(msg.format(test_loss, test_acc))
    content_write = ""
    content_write += f"Acc: {metrics_arr[0]}\t"
    content_write += f"Pre: {metrics_arr[1]}\t"
    content_write += f"Rec: {metrics_arr[2]}\t"
    content_write += f"F1s: {metrics_arr[3]}\t"
    print(content_write)
...
def evaluate(config, model, data_iter, test=False):
    ...
    pre = metrics.precision_score(labels_all, predict_all)
    recall = metrics.recall_score(labels_all, predict_all)
    F1_score = metrics.f1_score(labels_all, predict_all)
    metrics_arr = [acc, pre, recall, F1_score]

    if test:
        report = metrics.classification_report(labels_all, predict_all, target_names=config.class_list, digits=4)
        confusion = metrics.confusion_matrix(labels_all, predict_all)
        return metrics_arr, loss_total / len(data_iter), report, confusion
